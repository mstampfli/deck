...
| `config_edit.rs` | `deck config` mutations of project config, including whole-document `apply`. |
...
| `init.rs` | Project-aware `deck init`: seeds `deck.toml` from detected commands in one write. |
| `manifest.rs` | Manifest for `deck --json --help`, generated by walking the clap command graph. |
...
Today there are none: even the machine manifest is the JSON rendering of
`--help` (`deck --json --help`), generated by walking the clap command graph in
`manifest.rs` so it cannot drift from the parser. A unit test fails when a
command lacks its output-type annotation. The usual bootstrap sequence is:

- `deck --json --help`: discover commands, argv shapes, and output types.
